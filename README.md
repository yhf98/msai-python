...
pip install git+ssh://git@github.com/stainless-sdks/msai-python.git
```

> [!NOTE]
> Once this package is [published to PyPI](https://app.stainlessapi.com/docs/guides/publish), this will become: `pip install --pre msai`
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `msai.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `msai.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `msai.APIError`.
...
except msai.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except msai.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except msai.APIStatusError as e:
...
These methods return an [`APIResponse`](https://github.com/stainless-sdks/msai-python/tree/main/src/msai/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/msai-python/tree/main/src/msai/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/msai-python/issues) with questions, bugs, or suggestions.
